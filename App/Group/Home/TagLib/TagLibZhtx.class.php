...
//自定义标签库
class TagLibzhtx extends TagLib {
	...
	protected $tags = array(
		...
		'username'=>array('close'=>0),
		'mobile'=>array('close'=>0),
	...
	public function _artlist($attr, $content) {
		...
		$orderby = empty($attr['orderby'])? 'ordernum DESC,id DESC' : $attr['orderby'];
		...
		$pagetheme = empty($attr['pagetheme'])? ' %upPage% 第%nowPage%页/共%totalPage%页 %downPage%' : htmlspecialchars_decode($attr['pagetheme']);//新增加20140513
...
<?php
	...
	\$_artlist = D2('ArcView','article')->where(\$where)->order("$orderby")->limit(\$limit)->select();
...
str;
	...
	public function _username($attr,$content) {
		return C('cfg_username');
	}
	public function _phone($attr, $content) {
		return C('cfg_phone');
	}
	public function _mobile($attr, $content){
		return C('cfg_mobile');
	}
