...
<div class="main">
    ...
    <div class="operate">
        ...
        <input type="button" onclick="doGoSubmit('<?php echo U(GROUP_NAME. '/Category/sort');?>','form_do')" class="btn_blue" value="更新排序"></div>
    </div>
    <div class="list">    
    <form action="<?php echo U(GROUP_NAME.'/Category/sort');?>" method="post" id="form_do" name="form_do">
        <table width="100%">
			...
			<?php if(is_array($cate)): foreach($cate as $key=>$v): ?><tr>
                ...
                <td><?php if($v['status']): ?>是<?php else: ?><span style="color:red;">否</span><?php endif; ?></td>
				<td><?php if($v['status'] && $v['isnav']): ?>是<?php else: ?><span style="color:red;">否</span><?php endif; ?></td>
                ...
                <a href="javascript:;" onclick="toConfirm('<?php echo U(GROUP_NAME. '/Category/del', array('id' => $v['id']));?>', '确实要删除吗？')">删除</a>
				</td>
