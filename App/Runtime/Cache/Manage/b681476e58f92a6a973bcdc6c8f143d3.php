...
<div class="top_logo"> <img src="__PUBLIC__/images/main/logo.png" /> </div>
	 ...
	 <div class="top_link">
      <ul>
	    <li id="i_self">你好，<?php echo (session('zhtx_adm_username')); ?></li>
		<li id="i_hide_menu"><a href="#" id="togglemenu">隐藏菜单</a></li>
        <li id="i_home"><a href="<?php echo C('cfg_weburl');?>" target="_blank">首页</a></li> 
        <li id="i_help"><a href="<?php echo goLinkEncode('http://www.renrenbang.com/');?>" target="_blank">帮助</a></li>    
...
<?php if(is_array($menu)): foreach($menu as $k=>$v): ?><div id="items_left_menu_<?php echo ($k); ?>">
	<?php if(is_array($v["child"])): $k2 = 0; $__LIST__ = $v["child"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v2): $mod = ($k2 % 2 );++$k2;?><dl id="dl_items_<?php echo ($k2); ?>_<?php echo ($k); ?>">
      ...
      <?php if($v2['id'] == 7): if(is_array($qmenu)): $k3 = 0; $__LIST__ = $qmenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v3): $mod = ($k3 % 2 );++$k3;?><li><a href="<?php echo U(GROUP_NAME.'/'.$v3['module'].'/'.$v3['action'].'', $v3['parameter']);?>" target="main"><?php echo ($v3["name"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>  
      <?php else: ?>
      <?php if(is_array($v2["child"])): $k3 = 0; $__LIST__ = $v2["child"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v3): $mod = ($k3 % 2 );++$k3;?><li><a href="<?php echo U(GROUP_NAME.'/'.$v3['module'].'/'.$v3['action'].'', $v3['parameter']);?>" target="main"><?php echo ($v3["name"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; endif; ?>
		</ul>
		</dd>
