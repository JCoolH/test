...
// 				右边的/一定要去除
	...
	case 4:
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {			
			...
			$datafile = $setting['add_test'] == 1 ? './inc/zhtxcms_data.sql' : './inc/zhtxcms.sql';
			...
			$content=preg_replace('/\/\*[\w\W]*?\*\//', '', $content);
       		$content=preg_replace('/-- ----------------------------[\w\W]*?-- ----------------------------/', '', $content);
			$content = str_replace(
					array('#zhtx#_','#web_url#','#web_name#',"\r"), 
			...
			@file_get_contents('http://www.renrenbang.com/index.php?g=Api&m=Cms&a=getInstallInfo&email='.base64_encode($setting['email']));
			
			$status = 'success_all';
			$info .='ZHTXCMS'.$lang['installation_successful'];
